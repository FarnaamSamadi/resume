...
const Navbar = ({ onLanguageChange, lan, isOpen, toggleDropdown }) => {
  const items = {
    en: {
      intro: "Intro",
      about: "About",
  ...
  function lanActiveClass(language) {
    return language === lan ? "font-semibold text-lg" : "text-gray-400";
  }

  function generateDropdownClasses(classes) {
  ...
  let navsClass =
    "hidden lg:flex text-white text-xl px-3 py-2 flex-row items-center justify-between direction-rtl";

  return (
    <header className="sticky bg-gray-800 top-0">
      <div className="flex flex-row items-center justify-between px-3 py-2">
        <div className="w-16">
          <svg viewBox="0 0 512 512">
            ...
            <path
              fill="#9ad8ff"
              d="M371.873 400.656L307.589 448l8.858-79.344z"
            />
        ...
        <div className={(navsClass += lan === "fa" ? " direction-rtl" : "")}>
        ...
        <div className="px-3 py-2 lg:hidden" onClick={toggleDropdown}>
          {!isOpen && (
            <i
              className="text-gray-500 hover:text-white fa fa-bars fa-2x"
              aria-hidden="true"
            ></i>
          )}
          {isOpen && (
            <i
              className="relative z-10 text-gray-500 hover:text-white fa fa-times fa-2x"
              aria-hidden="true"
            ></i>
          )}
        </div>
      ...
      <div
        className={generateDropdownClasses(
          "absolute z-10 px-1 pb-3 pt-2 text-white bg-gray-800 w-full font-semibold font-mono lg:hidden"
        )}
      >
        {Object.keys(navItems).map(item => (
          <NavItem key={item} to={item} itemName={navItems[item]} />
